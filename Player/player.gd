...
var hitpoints: int = max_hitpoints:
	set(value):
		...
		health_label.modulate = Color.RED.lerp(Color.WHITE, float(hitpoints) / float(max_hitpoints))
		if hitpoints <= 0:
...
@onready var level_finish_menu: Control = $LevelFinishMenu
...
func _ready() -> void:
	...
	health_label.text = str(max_hitpoints) + "/" + str(max_hitpoints)
